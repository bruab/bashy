...
(function() {
  var BashyOS, BashySprite, DisplayManager, File, FileSystem, MenuManager, SoundManager, Task, TaskManager, Util, calculateChildCoords, cleanPath, createBashySprite, drawChildren, drawFile, drawFileSystem, drawLines, executeCommand, getParentPath, get_tasks, helpScreen, playIntro, showHomeText, showMediaText, showRootText, startGame, startTicker,
  ...
  executeCommand = function(command, args, os, task_mgr, display_mgr, sound_mgr) {
    var cwd, fs, ref, stderr, stdout;
    fs = os.file_system;
    ref = os.runCommand(command, args), cwd = ref[0], stdout = ref[1], stderr = ref[2];
    task_mgr.update(os);
  ...
  startGame = function(util, sound_mgr, stage, bashy_himself) {
    ...
    handleInput = function(input) {
      ...
      ref = util.parseCommand(input), command = ref[0], args = ref[1];
      if (indexOf.call(os.validCommands(), command) < 0) {
        return "Invalid command: " + command;
      } else {
        return executeCommand(command, args, os, task_mgr, display_mgr, sound_mgr);
      }
    };
  ...
  jQuery(function() {
    var bashy_himself, canvas, seenIntro, sound_mgr, stage, util;
    util = new Util();
    ...
    return bashy_himself.onload = function() {
      return startGame(util, sound_mgr, stage, bashy_himself);
    };
  });
  ...
  Util = (function() {
    ...
    Util.prototype.parseCommand = function(input) {
